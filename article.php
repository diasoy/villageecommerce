...
<div class="lg:px-72 xl:px-96 px-6 md:px-28 py-36 text-indigo-800">
    ...
    <div class="mb-8">
        <div class="flex gap-4 justify-center flex-wrap">
            <a href="<?= BASE_URL . "index.php?page=article&kategori=semua" ?>" class="px-4 py-2 rounded <?= ($kategori == 'semua') ? 'bg-indigo-700 text-white' : 'bg-white text-indigo-800' ?>">Semua Artikel</a>
            <a href="<?= BASE_URL . "index.php?page=article&kategori=tips" ?>" class="px-4 py-2 rounded <?= ($kategori == 'tips') ? 'bg-indigo-700 text-white' : 'bg-white text-indigo-800' ?>">Tips Bisnis</a>
            <a href="<?= BASE_URL . "index.php?page=article&kategori=kebijakan" ?>" class="px-4 py-2 rounded <?= ($kategori == 'kebijakan') ? 'bg-indigo-700 text-white' : 'bg-white text-indigo-800' ?>">Kebijakan</a>
            <a href="<?= BASE_URL . "index.php?page=article&kategori=inspirasi" ?>" class="px-4 py-2 rounded <?= ($kategori == 'inspirasi') ? 'bg-indigo-700 text-white' : 'bg-white text-indigo-800' ?>">Cerita Inspirasi</a>
            <a href="<?= BASE_URL . "index.php?page=article&kategori=kasus" ?>" class="px-4 py-2 rounded <?= ($kategori == 'kasus') ? 'bg-indigo-700 text-white' : 'bg-white text-indigo-800' ?>">Bedah Kasus</a>
        </div>
    </div>
