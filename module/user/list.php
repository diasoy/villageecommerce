...
if (mysqli_num_rows($queryAdmin) == 0) {
    echo "<h3 class='text-center text-lg font-bold py-52 w-full'>Saat ini belum ada data user yang dimasukan</h3>";
} else {
    ...
    echo "<tr class='border-b p-1 bg-green-600 text-white'>
                    ...
                    <th class='w-1/12 text-center py-2 pr-10'>Action</th>
                 </tr>";
    ...
    while ($rowUser = mysqli_fetch_array($queryAdmin)) {
        echo "<tr class='border-b p-1'>
                        ...
                        <td class='w-1/12 text-center py-3 pr-10'><a class='inline-block bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-1 px-5 rounded' href='" . BASE_URL . "index.php?page=my_profile&module=user&action=form&id_user=$rowUser[id_user]" . "'>Edit</a></td>
                     </tr>";
